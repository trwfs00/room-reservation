...
export default NextAuth({
    ...
    providers: [
        CredentialsProvider({
            ...
            authorize: async (credentials) => {
                ...
                const user = await User.findOne({ email: credentials.email }).select('+password +fullName +tel +role');

                if(!user) { throw new Error('ไม่พบผู้ใช้งานที่ใช้อีเมล หรือรหัสผ่านที่ตรงกันนี้.')}

                // Use the comparePassword method we defined in our user.js Model file to authenticate
                const pwValid = await user.comparePassword(credentials.password)

                if(!pwValid){ throw new Error("รหัสผ่านของคุณผิด") }
  ...
  pages: {
    // Here you can define your own custom pages for login, recover password, etc.
      signIn: '/signup', // we are going to use a custom login page (we'll create this in just a second)
  },
})